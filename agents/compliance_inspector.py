...
log = get_logger("compliance_inspector")

class ComplianceInspector:
    def __init__(self):
        self.llm = get_llm_service()

    def inspect_emails(self, emails: List[EmailInput]) -> List[EmailAnalysis]:
        ...
        for e in [e for e in emails if not (e.body and e.body.strip())]:
            results.append(self._normal_result(e.email_id, "No content found"))
        ...
        for i in range(0, len(valid), batch_size):
            ...
            try:
                ...
                results.extend([self._normal_result(e.email_id, "Processing error") for e in chunk])
        return results

    def _parse_results(self, raw: Dict[str, Any], emails: List[EmailInput]) -> List[EmailAnalysis]:
        res_list, final = raw.get("results", []), []
        LEVELS = {"l1": "low", "l2": "medium", "l3": "high", "l4": "critical"}
        ...
        for email, res in zip(emails, res_list + [{}] * (len(emails) - len(res_list))):
            try:
                ...
                final.append(EmailAnalysis(
                    ...
                    confidence=conf,
                    evidence_lines=ev,
                    reasoning=res.get("reasoning", "Standard inspection completed"),
                    manual_review_required=review,
                    manual_review_reason=res.get("manual_review_reason")
                ))
            except: final.append(self._normal_result(email.email_id, "Data extraction failed"))
        return final
